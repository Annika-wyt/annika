...
class ricatti_estimation():
    def __init__(self):
        ...
        self.estpose = np.array([-1.7, -0.9, 0], dtype=np.float64)
        self.estori = np.array([0.695, 0, 0.01, -0.718], dtype=np.float64) #w, x, y, z
    ...
    def pubOdom2(self, timeStamp):
        ...
        msg.transform.rotation.w = self.initori[0]
        msg.transform.rotation.x = self.initori[1]
        msg.transform.rotation.y = self.initori[2]
        msg.transform.rotation.z = self.initori[3]
